function fetchUpdatePriceNQtyCMP() {
  var databaseSpreadsheetId = '1pGH0ac47rmIOpQNMZ7sYeavn4Mrl3eC68gK8d7O-DN4';
  var databaseSheetName = 'CMP_DATA';
  var CMPAmazonSheet = 'CMP';

  var priceSpreadsheet = SpreadsheetApp.getActiveSpreadsheet();
  var priceSheet = priceSpreadsheet.getSheetByName(CMPAmazonSheet);
  ...
  submitPriceAndInventoryFeedForCMP();
}
...
function submitPriceAndInventoryFeedForCMP() {
  var feedContent = constructFeedContentForCMP();
  ...
  logSheet.getRange('A' + lastRow).setValue("CMP");
}

function confirmAndDownloadProcessingForCMP() {
  var lastRow = logSheet.getLastRow();
...
function constructFeedContentForCMP() {
  var sheet = spreadsheet.getSheetByName('CMP');
