function fetchUpdatePriceNQtyFGXCA() {
  var databaseSpreadsheetId = '1pGH0ac47rmIOpQNMZ7sYeavn4Mrl3eC68gK8d7O-DN4';
  var databaseSheetName = 'FGX_DATA';
  var FGXCAAmazonSheet = 'FGX_CA';

  var priceSpreadsheet = SpreadsheetApp.getActiveSpreadsheet();
  var priceSheet = priceSpreadsheet.getSheetByName(FGXCAAmazonSheet);
  ...
  submitPriceAndInventoryFeedForFGXCA();
}
...
function submitPriceAndInventoryFeedForFGXCA() {
  var feedContent = constructFeedContentForFGXCA();
  ...
  logSheet.getRange('A' + lastRow).setValue("FGX_CA");
}

function confirmAndDownloadProcessingForFGXCA() {
  var lastRow = logSheet.getLastRow();
...
function constructFeedContentForFGXCA() {
  var sheet = spreadsheet.getSheetByName('FGX_CA');
