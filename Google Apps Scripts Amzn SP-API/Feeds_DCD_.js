function fetchUpdatePriceNQtyDCD() {
  var databaseSpreadsheetId = '1pGH0ac47rmIOpQNMZ7sYeavn4Mrl3eC68gK8d7O-DN4';
  var databaseSheetName = 'DCD_DATA';
  var DCDAmazonSheet = 'DCD';

  var priceSpreadsheet = SpreadsheetApp.getActiveSpreadsheet();
  var priceSheet = priceSpreadsheet.getSheetByName(DCDAmazonSheet);
  ...
  for (var i = 1; i < databaseValues.length; i++) {
    ...
    mapLookup[key] = databaseValues[i][8];
  }
  ...
  for (var j = 1; j <= numRows; j++) {
    ...
    var newMAP = mapLookup[lookupKey] !== undefined ? mapLookup[lookupKey] : row[8];

    updatedPrices.push([newPrice]);
    updatedQtys.push([newQty]);
    updatedMAPs.push([newMAP]);

    var status = (newQty > 1 && newPrice <= 149) ? "ENABLED" : "DISABLED";
  ...
  priceSheet.getRange(2, 9, numRows, 1).setValues(updatedMAPs);    // Column I
  ...
  submitPriceAndInventoryFeedForDCD();
}
...
function submitPriceAndInventoryFeedForDCD() {
  var feedContent = constructFeedContentForDCD();
  ...
  logSheet.getRange('A' + lastRow).setValue("DCD");
}

function confirmAndDownloadProcessingForDCD() {
  var lastRow = logSheet.getLastRow();
...
function constructFeedContentForDCD() {
