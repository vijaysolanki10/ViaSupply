function fetchUpdatePriceNQtyTPS() {
  var databaseSpreadsheetId = '1pGH0ac47rmIOpQNMZ7sYeavn4Mrl3eC68gK8d7O-DN4';
  var databaseSheetName = 'TPS_DATA';
  var TPSAmazonSheet = 'TPS';

  var priceSpreadsheet = SpreadsheetApp.getActiveSpreadsheet();
  var priceSheet = priceSpreadsheet.getSheetByName(TPSAmazonSheet);
  ...
  submitPriceAndInventoryFeedForTPS();
}
...
function submitPriceAndInventoryFeedForTPS() {
  var feedContent = constructFeedContentForTPS();
  ...
  logSheet.getRange('A' + lastRow).setValue("TPS");
}

function confirmAndDownloadProcessingForTPS() {
  var lastRow = logSheet.getLastRow();
...
function constructFeedContentForTPS() {
  var sheet = spreadsheet.getSheetByName('TPS');
