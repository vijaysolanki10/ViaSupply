function fetchUpdatePriceNQtyDCDCA() {
  ...
  submitPriceAndInventoryFeedForDCDCA();
}
...
function submitPriceAndInventoryFeedForDCDCA() {
  var feedContent = constructFeedContentForDCDCA();
  ...
  logSheet.getRange('A' + lastRow).setValue("DCD_CA");
}

function confirmAndDownloadProcessingForDCDCA() {
  var lastRow = logSheet.getLastRow();
  var feedId = logSheet.getRange('I' + lastRow).getValue();
  confirmFeedProcessingCA(feedId); // Assuming you want to use the same confirmation function
}

function constructFeedContentForDCDCA() {
  var sheet = spreadsheet.getSheetByName('DCD_CA');
